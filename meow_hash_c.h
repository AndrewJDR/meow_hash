/* ========================================================================
   ...
   ========================================================================
   Source below is heavily modified to port to generic C.
   ======================================================================== */
...
static const uint32_t meow_aes_box0[256] = {
...
static const uint32_t meow_aes_box1[256] = {
...
static const uint32_t meow_aes_box2[256] = {
...
static const uint32_t meow_aes_box3[256] = {
...
// this assumes little-endian and 4-byte unanaligned loads are OK
static void meow_aes_op(uint32_t* state, const void* key)
{
    uint32_t s0 = state[0];
    uint32_t s1 = state[1];
    uint32_t s2 = state[2];
    ...
    state[0] = meow_aes_box0[s0 & 0xff] ^ meow_aes_box1[s1 >> 24] ^ meow_aes_box2[(s2 >> 16) & 0xff] ^ meow_aes_box3[(s3 >> 8) & 0xff] ^ key32[0];
    state[1] = meow_aes_box0[s1 & 0xff] ^ meow_aes_box1[s2 >> 24] ^ meow_aes_box2[(s3 >> 16) & 0xff] ^ meow_aes_box3[(s0 >> 8) & 0xff] ^ key32[1];
    state[2] = meow_aes_box0[s2 & 0xff] ^ meow_aes_box1[s3 >> 24] ^ meow_aes_box2[(s0 >> 16) & 0xff] ^ meow_aes_box3[(s1 >> 8) & 0xff] ^ key32[2];
    state[3] = meow_aes_box0[s3 & 0xff] ^ meow_aes_box1[s0 >> 24] ^ meow_aes_box2[(s1 >> 16) & 0xff] ^ meow_aes_box3[(s2 >> 8) & 0xff] ^ key32[3];
}
...
MEOW_API
meow_hash meow_hash_c(meow_u64 seed, meow_u64 length, const void* data)
{
    meow_hash S0 = {};
    meow_hash S1 = {};
    meow_hash S2 = {};
    ...
    while (blocks--)
    {
        meow_aes_op(S0.u32, bytes);
        meow_aes_op(S1.u32, bytes + 16);
        meow_aes_op(S2.u32, bytes + 32);
        meow_aes_op(S3.u32, bytes + 48);
        meow_aes_op(S4.u32, bytes + 64);
        meow_aes_op(S5.u32, bytes + 80);
        meow_aes_op(S6.u32, bytes + 96);
        meow_aes_op(S7.u32, bytes + 112);
        meow_aes_op(S8.u32, bytes + 128);
        meow_aes_op(S9.u32, bytes + 144);
        meow_aes_op(SA.u32, bytes + 160);
        meow_aes_op(SB.u32, bytes + 176);
        meow_aes_op(SC.u32, bytes + 192);
        meow_aes_op(SD.u32, bytes + 208);
        meow_aes_op(SE.u32, bytes + 224);
        meow_aes_op(SF.u32, bytes + 240);
    ...
    switch (length >> 4)
    {
        case 15: meow_aes_op(SE.u32, bytes + 224);
        case 14: meow_aes_op(SD.u32, bytes + 208);
        case 13: meow_aes_op(SC.u32, bytes + 192);
        case 12: meow_aes_op(SB.u32, bytes + 176);
        case 11: meow_aes_op(SA.u32, bytes + 160);
        case 10: meow_aes_op(S9.u32, bytes + 144);
        case  9: meow_aes_op(S8.u32, bytes + 128);
        case  8: meow_aes_op(S7.u32, bytes + 112);
        case  7: meow_aes_op(S6.u32, bytes + 96);
        case  6: meow_aes_op(S5.u32, bytes + 80);
        case  5: meow_aes_op(S4.u32, bytes + 64);
        case  4: meow_aes_op(S3.u32, bytes + 48);
        case  3: meow_aes_op(S2.u32, bytes + 32);
        case  2: meow_aes_op(S1.u32, bytes + 16);
        case  1: meow_aes_op(S0.u32, bytes);
        default:;
    }
    ...
    if (length & 0xF)
    {
        if (total >= 16)
        {
            meow_aes_op(SF.u32, bytes + length - 16);
        }
        else
        {
            uint8_t partial[16] = {};
            ...
            meow_aes_op(SF.u32, partial);
        }
    }

    meow_hash M0 = S7;
    meow_aes_op(M0.u32, SA.u32);
    meow_aes_op(M0.u32, S4.u32);
    meow_aes_op(M0.u32, S5.u32);
    meow_aes_op(M0.u32, SC.u32);
    meow_aes_op(M0.u32, S8.u32);
    meow_aes_op(M0.u32, S0.u32);
    meow_aes_op(M0.u32, S1.u32);
    meow_aes_op(M0.u32, S9.u32);
    meow_aes_op(M0.u32, SD.u32);
    meow_aes_op(M0.u32, S2.u32);
    meow_aes_op(M0.u32, S6.u32);
    meow_aes_op(M0.u32, SE.u32);
    meow_aes_op(M0.u32, S3.u32);
    meow_aes_op(M0.u32, SB.u32);
    meow_aes_op(M0.u32, SF.u32);
    
    uint64_t mixer[] = { seed - total, seed + total + 1 };
    
    meow_aes_op(M0.u32, mixer);
    meow_aes_op(M0.u32, mixer);
    meow_aes_op(M0.u32, mixer);
